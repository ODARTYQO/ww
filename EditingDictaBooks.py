...
#---------------------חיפוש והחלפה----------------
    ...
    def on_text_edit_changed(self):
        ...
        try:
            # שמירת התוכן המקורי כולל תגיות HTML
            content = self.text_display.document().toRawText()
                
            # שמירה לקובץ תוך שמירה על התגיות
            with open(self.current_file_path, 'w', encoding='utf-8') as file:
                file.write(content)
            
            # עדכון ההיסטוריה
            self._safe_update_history(content, "עריכת טקסט")
